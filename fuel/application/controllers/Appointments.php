...
class Appointments extends CI_Controller {
    ...
    public function ajax_register_appointment() {
        ...
        try {
            ...
            echo json_encode(array(
                    ...
                    'msg'=>'<br>Your pre-booking code: TYHG78<br><br>Please show this or your confirmation email to a member of the pharmacy staff<br><br>'.$customer['first_name'] . ' ' . $customer['last_name'].'<br><br>Appointment '. date('d/m/Y H:i', strtotime($appointment['start_datetime'])).' at '.$clinic['cl_clinic_name'].',  '.$clinic['cl_formatted_address'].'<br><br>You have also been sent a confirmation email with these details on it.<br><br> Amount Billed: &pound;'.($price/100) 
                ));
	...
	protected function _get_provider_available_time_periods($provider_id, $selected_date,
			$exclude_appointments = array(), $clinic_id) {
		$this->load->model('appointments_model');
	    $this->load->model('providers_model');
