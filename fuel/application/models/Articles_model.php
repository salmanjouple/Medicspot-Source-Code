...
class Articles_model extends Base_module_model {
	...
	{
		$this->db->join('teams', 'teams.id = articles.author_id', 'left');
		$this->db->select('articles.id, articles.title, summary, SUBSTRING(content, 1, 50) AS content, teams.name AS author, date_added, articles.published', FALSE);
...
class Article_model extends Base_module_record {
	...
	{
		return site_url('images/articles/'.$this->slug);
	}
