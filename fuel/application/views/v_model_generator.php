...
<div id="container">
	...
	<div id="body">
		...
		<p>Create All Models Automatically for All Tables.</p>
		<form action="<?php echo base_url(); ?>model_generator/create_all_models" method="post">
		<p><input type="submit" value="Create All Models" /></p>
		</form>
		<?php
		if(isset($_GET['success'])) {
            $path_to_model = realpath(dirname(__FILE__) . '/../models');
			echo '<div style="color:green;font-weight:bold;" >Model Created!</div><code><i>Directory: '.$path_to_model.'\\'.$_GET['table'].'_model.php </i></code>';
		}
		if(isset($_GET['success_all'])) {
            $path_to_model = realpath(dirname(__FILE__) . '/../models');
			echo '<div style="color:green;font-weight:bold;" >All Models Created!</div><code><i>Directory: '.$path_to_model.'</i></code>';
		}
		if(isset($_GET['error'])){
